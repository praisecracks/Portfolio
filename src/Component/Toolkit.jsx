...
// Upgraded Tool Card with smooth 3D tilt, glow, and spring animation
function ToolCard({ tool, index }) {
  ...
  const handleMouseMove = (e) => {
    ...
    const x = ((e.clientX - rect.left) / rect.width - 0.5) * 20; // increased tilt
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      transition={{ delay: index * 0.05, duration: 0.6, type: 'spring', stiffness: 120 }}
      onMouseMove={handleMouseMove}
      onMouseLeave={resetTilt}
      style={{ transform: `perspective(1000px) rotateX(${tilt.y}deg) rotateY(${tilt.x}deg)` }}
      whileHover={{ scale: 1.07, boxShadow: '0 20px 35px rgba(20,184,166,0.3)' }}
      className="relative backdrop-blur-md bg-white/70 dark:bg-gray-800/70 border border-white/20 dark:border-gray-700 rounded-xl p-6 cursor-pointer transition-transform duration-300 hover:shadow-xl"
    >
